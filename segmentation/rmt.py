...
class VisionRetentionChunk(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rel_pos, chunkwise_recurrent=False, incremental_state=None):
        ...
        k *= self.scaling
        q = q.view(bsz, h, w, self.num_heads, self.key_dim).permute(0, 3, 1, 2,
                                                                    4)  # (b n h w d1), torch.Size([10, 4, 56, 56, 16])
        k = k.view(bsz, h, w, self.num_heads, self.key_dim).permute(0, 3, 1, 2,
                                                                    4)  # (b n h w d1), torch.Size([10, 4, 56, 56, 16])
        ...
        qr_w = qr.transpose(1, 2)  # (b h n w d1), torch.Size([10, 56, 4, 56, 16])
        kr_w = kr.transpose(1, 2)  # (b h n w d1), torch.Size([10, 56, 4, 56, 16])
        v = v.reshape(bsz, h, w, self.num_heads, -1).permute(0, 1, 3, 2,
                                                             4)  # (b h n w d2), torch.Size([10, 56, 4, 56, 16])

        qk_mat_w = qr_w @ kr_w.transpose(-1, -2)  # (b h n w w), , torch.Size([10, 56, 4, 56, 56])
        qk_mat_w = qk_mat_w + mask_w  # (b h n w w), torch.Size([10, 56, 4, 56, 56])
        qk_mat_w = torch.softmax(qk_mat_w, -1)  # (b h n w w)
        v = torch.matmul(qk_mat_w, v)  # (b h n w d2), torch.Size([10, 56, 4, 56, 16])

        qr_h = qr.permute(0, 3, 1, 2, 4)  # (b w n h d1), torch.Size([10, 56, 4, 56, 16])
        kr_h = kr.permute(0, 3, 1, 2, 4)  # (b w n h d1), torch.Size([10, 56, 4, 56, 16])
        v = v.permute(0, 3, 2, 1, 4)  # (b w n h d2), torch.Size([10, 56, 4, 56, 16])

        qk_mat_h = qr_h @ kr_h.transpose(-1, -2)  # (b w n h h), torch.Size([10, 56, 4, 56, 56])
        qk_mat_h = qk_mat_h + mask_h  # (b w n h h), torch.Size([10, 56, 4, 56, 56])
        qk_mat_h = torch.softmax(qk_mat_h, -1)  # (b w n h h)
        output = torch.matmul(qk_mat_h, v)  # (b w n h d2), torch.Size([10, 56, 4, 56, 16])
        ...
        if getattr(self, "__DEBUG__", False):
            setattr(self, "__data__", dict(mask_w=mask_w, mask_h=mask_h, qr_w=qr_w, kr_w=kr_w, qr_h=qr_h, kr_h=kr_h))
...
class VisionRetentionAll(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rel_pos, chunkwise_recurrent=False, incremental_state=None):
        ...
        rel_pos: mask: (n l l)
        '''
        bsz, h, w, _ = x.size()  # torch.Size([10, 7, 7, 512])
        (sin, cos), mask = rel_pos
        # sin: torch.Size([7, 7, 32])
        # cos: torch.Size([7, 7, 32])
        # mask: torch.Size([16, 49, 49])
        ...
        qk_mat = qk_mat + mask  # (b n l l), torch.Size([10, 16, 49, 49])
...
class Backbone_VisRetNet(VisRetNet):
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            print("incompatibleKeys", incompatibleKeys)
            time.sleep(5)
    ...
    def forward(self, x):
        def layer_forward(layer, x):
            b, h, w, d = x.size()
            rel_pos = layer.Relpos((h, w), chunkwise_recurrent=layer.chunkwise_recurrent)
            # (sin_, cos_), _ = rel_pos
            # print('**** rel_pos ', type(rel_pos), len(rel_pos))
            # print('**** sin_ ', sin_.shape, cos_.shape)

            for blk in layer.blocks:
                x = blk(x, incremental_state=None, chunkwise_recurrent=layer.chunkwise_recurrent,
                        retention_rel_pos=rel_pos)
...
class rmt_tiny(Backbone_VisRetNet):
...
@BACKBONES.register_module()
class rmt_small(Backbone_VisRetNet):
    def __init__(self, **kwargs):
        super().__init__(
        out_indices=(0, 1, 2, 3),
        pretrained=kwargs['pretrained'],
...
@BACKBONES.register_module()
class rmt_base(Backbone_VisRetNet):
    def __init__(self, **kwargs):
        super().__init__(
        out_indices=(0, 1, 2, 3),
        pretrained=kwargs['pretrained'],
...
if __name__ == "__main__":
    model = Backbone_VisRetNet().cuda()
    x = torch.randn([10, 3, 224, 224]).cuda()

    outs = model(x)
    print("outs", len(outs), [x.shape for x in outs])
    # [torch.Size([10, 64, 56, 56]), torch.Size([10, 128, 28, 28]), torch.Size([10, 256, 14, 14]), torch.Size([10, 512, 7, 7])]
