...
def train_one_epoch(lr_scheduler, model: torch.nn.Module, criterion: DistillationLoss,
                    ...
                    set_training_mode=True, logger=None, mesa=-1):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if not math.isfinite(loss_value):
            logger.info("Epoch: [{}], Loss is {}, stopping training".format(epoch, loss_value))
            sys.exit(1)
        optimizer.zero_grad()
    ...
    logger.info(f"Epoch: [{epoch}], Averaged stats: {metric_logger}")
